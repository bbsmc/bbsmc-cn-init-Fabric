...
public class HostingPackage {
    ...
    public static void loadAsync() {
        CompletableFuture.runAsync(() -> {
            if (loadFromRemote()) {
                Ytongame_hostingmenuClient.LOGGER.info("Loaded {} hosting packages from remote", ALL_PACKAGES.size());
                isLoaded = true;
                return;
            }

            Ytongame_hostingmenuClient.LOGGER.warn("Failed to load from remote, falling back to local resources");
            loadFromResources();
            isLoaded = true;
        });
    }
    ...
    private static boolean loadFromRemote() {
        ...
        try {
            ...
            connection.setRequestProperty("User-Agent", "YtonGame-HostingMenu/" + Ytongame_hostingmenuClient.MODID);
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                Ytongame_hostingmenuClient.LOGGER.warn("Remote JSON returned status code: {}", responseCode);
                return false;
            }
        } catch (Exception e) {
            Ytongame_hostingmenuClient.LOGGER.warn("Failed to load hosting packages from remote: {}", e.getMessage());
            return false;
        } finally {
            if (connection != null) {
        ...
        try {
            ...
            if (is != null) {
                if (loadFromStream(is)) {
                    Ytongame_hostingmenuClient.LOGGER.info("Loaded {} hosting packages from resources", ALL_PACKAGES.size());
                } else {
                    loadDefaultPackages();
                }
            } else {
                Ytongame_hostingmenuClient.LOGGER.error("Could not find hosting_packages.json in resources");
                loadDefaultPackages();
            }
        } catch (Exception e) {
            Ytongame_hostingmenuClient.LOGGER.error("Failed to load hosting packages from resources", e);
            loadDefaultPackages();
        }
    }
    ...
    private static boolean loadFromStream(InputStream is) {
        try (InputStreamReader reader = new InputStreamReader(is, StandardCharsets.UTF_8)) {
            ...
            Ytongame_hostingmenuClient.LOGGER.error("Failed to parse hosting packages JSON", e);
            return false;
        }
    }
    ...
    private static void loadDefaultPackages() {
        ...
        Ytongame_hostingmenuClient.LOGGER.info("Loaded default hosting packages");
    }
}
